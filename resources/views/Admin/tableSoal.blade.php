                                    ...
                                    <tr
                                        ...
                                        <td class="px-6 py-4">
                                            @foreach ($a->role as $ar)
                                                @foreach ($getRole as $ro)
                                                    @if ($ro->id == $ar)
                                                        <span
                                                            class="bg-gradient-to-tl from-green-600 to-lime-400 px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white">{{ $ro->role }}</span>
                                                    @endif
                                                @endforeach
                                            @endforeach
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex space-x-4">
                                                ...
                                                <form action="{{ route('soals.destroy', $a) }}" method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit"
